...
export async function describeRoomList(PageIndex, PageSize) {
  ...
  const url = `https://rtc-api.zego.im/?Action=DescribeRoomList&AppId=${ZEGOCLOUD_APP_ID}&SignatureNonce=${signatureNonce}&Timestamp=${timestamp}&Signature=${sig}&SignatureVersion=2.0&IsTest=false&PageIndex=${PageIndex}&PageSize=${PageSize}`;
